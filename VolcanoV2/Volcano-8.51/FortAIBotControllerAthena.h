...
namespace FortAIBotControllerAthena
{
    ...
    void SpawnPlayerBot(AActor* OverrideSpawnLoc = nullptr, PlayerBots::EBotState StartingState = PlayerBots::EBotState::Warmup) {
        ...
        // Use APlayerPawn_Athena_C as fallback if Phoebe pawn class is not available
        if (!PhoebePawnClass) {
            PhoebePawnClass = APlayerPawn_Athena_C::StaticClass();
            Log("Using default player pawn class for bots");
        }

        static int MaxSpawnLocations = 1;
        ...
        if (!GameMode || !GameState) {
            Log("GameMode or GameState is null!");
            return;
        }
        
        // Update max spawn locations if we have player starts now
        if (PlayerStarts.Num() > 0) {
            MaxSpawnLocations = PlayerStarts.Num();
        }
        ...
        // Get spawn location
        FVector SpawnLoc = FVector{0, 0, 5000};
        ...
        if (PlayerStarts.Num() > 0) {
            ...
            SpawnLoc = BotSpawn->K2_GetActorLocation();
            SpawnRot = BotSpawn->K2_GetActorRotation();
        }
        
        // Try to spawn using ServerBotManager if available
        AFortPlayerPawnAthena* Pawn = nullptr;
        if (GameMode->ServerBotManager) {
            Pawn = GameMode->ServerBotManager->SpawnBot(SpawnLoc, SpawnRot, BotCustomizationData);
        }
        
        // Fallback: spawn pawn manually if ServerBotManager failed or doesn't exist
        if (!Pawn) {
            Log("ServerBotManager spawn failed, using manual spawn");
            FTransform SpawnTransform{};
            SpawnTransform.Translation = SpawnLoc;
            SpawnTransform.Rotation = SpawnRot.Quaternion();
            SpawnTransform.Scale3D = FVector{1, 1, 1};
            Pawn = (AFortPlayerPawnAthena*)GetWorld()->SpawnActor(PhoebePawnClass, &SpawnLoc, &SpawnRot);
        }
        
        if (!Pawn) {
            Log("Failed to spawn bot pawn!");
            return;
        }
        ...
        if (!PC) {
            Log("Creating AI controller for bot");
            PC = (AFortAthenaAIBotController*)GetWorld()->SpawnActor(AFortAthenaAIBotController::StaticClass(), &SpawnLoc, &SpawnRot);
            if (PC) {
                PC->Possess(Pawn);
            }
        }
        ...
        if (!PC) {
            Log("Failed to get or create bot controller!");
            return;
        }
        
        AFortPlayerStateAthena* PlayerState = (AFortPlayerStateAthena*)PC->PlayerState;
        if (!PlayerState) {
            Log("Failed to get player state!");
            return;
        }
    ...
    void HookAll() {
        // NOTE: Offsets are for 10.40, need to find correct offsets for 8.51
        // For now, we'll use the bot system without hooking these functions
        // The bots will spawn and function with the built-in behavior tree
        ...
        //MH_CreateHook((LPVOID)(ImageBase + 0x1D2CED0), OnAgentDBNO, (LPVOID*)&OnAgentDBNOOG);

        Log("Bot system hooks ready (8.51 mode)!");
    }
}