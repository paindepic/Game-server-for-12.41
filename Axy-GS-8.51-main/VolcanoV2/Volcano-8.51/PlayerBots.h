...
namespace PlayerBots {
    ...
    public:
        ...
        AFortPickup* GetNearestPickup() {
...

            {
                ...
                AActor* Pickup = (AActor*)PickupArray[i];
        ...
        APawn* GetNearestPawn() {
            ...
            for (size_t i = 0; i < Globals::AliveBots.Num(); i++)
            {
                if (Globals::AliveBots[i]->Pawn != Pawn)
                {
                    if ((Globals::AliveBots[i]->Pawn && Globals::AliveBots[i]->Pawn->GetDistanceTo(Pawn) < NearestDistance))
                    {
                        if (Globals::AliveBots[i]->Pawn == Pawn) {
                            continue;
						}
                        AFortPlayerStateAthena* PS = (AFortPlayerStateAthena*)Globals::AliveBots[i]->PlayerState;
                        if (PS->TeamIndex != PlayerState->TeamIndex) {
							NearestDistance = Globals::AliveBots[i]->Pawn->GetDistanceTo(Pawn);
                            NearestPlayer = Globals::AliveBots[i]->Pawn;
                        }
                    }
                }
            }
        ...
        APawn* GetRandomPawn() {
			...
			APawn* RandomBot = Globals::AliveBots[GetMath()->RandomIntegerInRange(0, Globals::AliveBots.Num() - 1)]->Pawn;
            ...
            if (Pawn->CurrentWeapon->WeaponData->IsA(UFortWeaponMeleeItemDefinition::StaticClass()))
            {
                for (size_t i = 0; i < PC->Inventory->Inventory.ReplicatedEntries.Num(); i++)
                {
                    ...
                    if (Entry.ItemDefinition) {
                        ...
                        if (ItemName.contains("Shotgun") || ItemName.contains("SMG") || ItemName.contains("Assault")
                            || ItemName.contains("Sniper") || ItemName.contains("Rocket") || ItemName.contains("Pistol")) {
                            //Log(("SimpleSwitchToWeapon: " + ItemName).c_str());
    ...
    public:
        ...
        bool IsFullInventory(AFortAthenaAIBotController* PC)
        {
            ...
            for (int i = 0; i < PC->Inventory->Inventory.ReplicatedEntries.Num(); i++)
            {
                ...
                {
                    ++Count;
                    //Log((std::format("BotInventory::IsFullInventory Count={}", Count).c_str()).c_str());
                    if (Count >= Max)
                        return true;
                }
            }
        ...
        void DropItem(PlayerBot* bot, UFortItemDefinition* ItemDef) {
            ...
            SpawnPickup(ItemDef, ItemToDrop->Count, ItemToDrop->LoadedAmmo, Drop, EFortPickupSourceTypeFlag::Player, EFortPickupSpawnSource::Unset, );
        ...
        UFortItemDefinition* WantsToSwapItemFor(PlayerBot* bot, UFortItemDefinition* ItemDef) {
            ...
            if (ItemDefTargetedSwap) {
                ...
                /*if (SwapItemType > EBotSwapItemType::ShieldHeals && SwapItemType != EBotSwapItemType::Other && SwapItemType != EBotSwapItemType::MAX) {
                    Log(("ItemRarities: " + std::to_string((int).c_str())ItemDef->Rarity) + " SwapItemRarity: " + std::to_string((int)ItemDefTargetedSwap->Rarity));
                    if (ItemDef->Rarity > ItemDefTargetedSwap->Rarity && ItemDefSwapType == SwapItemType) {
                        Log(("ItemToSwap: " + ItemDefTargetedSwap->Name.ToString().c_str()));
                        return ItemDefTargetedSwap;
                    }
                }
                    ...
                    if ((ItemDefTargetedSwapName.contains("Small") || !ItemDefTargetedSwapName.contains("Mini")) &&
                        (ItemDefName.contains("Shield") && (!ItemDefName.contains("Small") || !ItemDefName.contains("Mini")))) {
                        Log(("ItemToSwap: " + ItemDefTargetedSwapName).c_str());
                        return ItemDefTargetedSwap;
                    }
                }
        ...
        AFortPickup* GetNearestFilteredPickup(PlayerBot* bot) {
...

            {
                ...
                AActor* PickupActor = (AActor*)PickupArray[i];
        ...
        void CompletePickupAnimation(PlayerBot* bot, AFortPickup* Pickup) {
            ...
            if (ItemToSwap) {
                ...
                if (SwapItemEntry && SwapItemEntry->ItemDefinition) {
                    Log(("ItemToSwap: " + SwapItemEntry->ItemDefinition->Name.ToString().c_str()));
                ...
                if (!IsFullInventory(bot->PC)) {
                    if (!bot || !bot->PC || !bot->PC->Inventory) {
                        Log(("Smth is null!").c_str());
                        SpawnPickup(PickupItemDefinition, PickupEntry.Count, PickupEntry.LoadedAmmo, Drop, EFortPickupSourceTypeFlag::Player, EFortPickupSpawnSource::Unset, );
                        return;
                    }
                ...
                if (IsFullInventory(bot->PC))
                {
                    ...
                    if (CurrentWeapon)
                    {
                        ...
                        if (OldWeaponDef && OldWeaponDef->IsA(UFortWeaponMeleeItemDefinition::StaticClass()))
                        {
                            ...
                            {
                                //Log(("False").c_str());
                                SpawnPickup(PickupItemDefinition, PickupCount, PickupLoadedAmmo, Drop, EFortPickupSourceTypeFlag::Player, EFortPickupSpawnSource::Unset, );
                            }
                        }
                        else
                        {
                            if (GetQuickBars(PickupItemDefinition) == EFortQuickBars::Primary)
                            {
                                ...
                                SpawnPickup(OldWeaponDef, OldWeaponCount, OldWeaponAmmo, Drop, EFortPickupSourceTypeFlag::Player, EFortPickupSpawnSource::Unset, );
    ...
    public:
        bool IsMaximumHealingReached(PlayerBot* bot, UFortItemDefinition* ItemDef) {
            ...
            //Log(("ItemDef: " + ItemName + " ReachedMaximum: " + std::to_string(bMaximumHealingReached).c_str()) + " Health: " + std::to_string(Health) + " Shield: " + std::to_string(Shield));
        ...
        void StartHealing(PlayerBot* bot) {
            ...
            //Log(("SwitchToHealingItem: " + HealingItemEntry.ItemDefinition->GetFullName().c_str()));
            ...
            if (bot->tick_counter % 10 == 0) {
                ...
                if (GameState->GamePhase >= EAthenaGamePhase::SafeZones) {
                    if (GameState->SafeZoneIndicator && !GameState->SafeZoneIndicator->NextCenter.IsZero()) {
                        ...
                        //Log(("DistanceToZone: " + std::to_string(DistanceToZone).c_str()) + " Radius: " + std::to_string(GameState->SafeZoneIndicator->NextRadius) + " bIsInSafeZone: " + std::to_string(bot->bIsInSafeZone));
                    }
                }
            }
    ...
    void TickBots() {
        ...
        for (size_t i = 0; i < PlayerBotArray.Num(); i++)


        {


            PlayerBot* Bot = (PlayerBot*)PlayerBotArray[i];
            ...
            if (Bot->Pawn->IsDead()) {
                for (int i = 0; i < PlayerBotArray.size(); i++) {
                    if (PlayerBotArray[i]->Pawn == Bot->Pawn) {
                        ...
                        Log(("Freed a dead bot from the array!").c_str());
                    }
                }
                continue;
            }
