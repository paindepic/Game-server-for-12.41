...
namespace FortAIBotControllerAthena
{
    ...
    AFortPlayerPawnAthena* SpawnBot(UFortServerBotManagerAthena* This, FVector SpawnLoc, FRotator SpawnRot, UFortAthenaAIBotCustomizationData* BotData)
    {
        ...
        if (!Backpacks.empty() && GetMath()->RandomBoolWithWeight(0.5)) { // less likely to equip than skin cause lots of ppl prefer not to use backpack
        ...
        if (!Contrails.empty() && GetMath()->RandomBoolWithWeight(0.95)) {
        ...
        if (BotData->BehaviorTree) {
            ...
            if (PC->RunBehaviorTree(BotData->BehaviorTree)) {
                ...
                PC->Blackboard->SetValueAsEnum(GetString()->Conv_StringToName(L"AIEvaluator_Global_GamePhaseStep"), (int)GameState->GamePhaseStep);
                ...
                if (BuildingFoundations.Num() > 0) {
                    ...
                    if (DropZone) {
                        PC->Blackboard->SetValueAsVector(GetString()->Conv_StringToName(L"AIEvaluator_JumpOffBus_Destination"), DropZone->K2_GetActorLocation());
                    }
                }
            }
            else {
