...
namespace Globals
{
    ...
    // Alive bots tracking (custom since AFortGameModeAthena::AliveBots doesn't exist in 8.51)
    TArray<AFortAthenaAIBotController*> AliveBots;

    // Global references for bot system
    TArray<AActor*> PlayerStarts;
    TArray<AActor*> BuildingFoundations;
    UAthenaNavSystem* AthenaNavSystem = nullptr;
    uintptr_t ImageBase = 0;

    // Building item definitions for bots
    UFortBuildingItemDefinition* Floor = nullptr;
    UFortBuildingItemDefinition* Roof = nullptr;
    UFortBuildingItemDefinition* Stair = nullptr;
    UFortBuildingItemDefinition* Wall = nullptr;
    UFortEditToolItemDefinition* EditTool = nullptr;
...
// Simple log function for bot system (takes const char*)
inline void Log(const char* message)
{
    LOG_(message);
}

// Stub functions for bot system ability management
inline void AbilitySpecConstructor(FGameplayAbilitySpec* Spec, UGameplayAbility* Ability, int32_t Level = 1, int32_t InputID = -1, UObject* SourceObject = nullptr)
{
    if (!Spec || !Ability)
        return;
    ...
    Spec->Handle.GenerateNewHandle();
}

inline void GiveAbilityAndActivateOnce(UAbilitySystemComponent* ASC, FGameplayAbilityActorInfo* ActorInfo, FGameplayAbilitySpec& Spec)
{
    if (!ASC || !Spec.Ability)
        return;

    ASC->GiveAbility(Spec.Handle);
